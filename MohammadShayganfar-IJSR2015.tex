...
Collaborative robots must be designed to address the two challenging aspects of
sociability and working with humans on shared activities in the same
environment. The existing prominent collaboration theories provide frameworks
...
of the important elements within a collaboration structure, there is still a lack
...
components of the collaboration structure, e.g., shared shared, provide the
...
\ref{sec:computational-framework} to show how each mechanism is involved to
...
it does not acknowledge the Astronaut' emotion, and second, it immediately
...
notice (for the same reason) that the underlined utterance in B2, reveals that
the Robot requires the Astronaut's confirmation for the next step.} As it
appears in the Astronaut's response, the Robot's immediate proposal does not
...
Astronaut's frustration (B4). The Robot's utterance creates an ambiguous
...
measurement tool and proposes the potential solution of replacing the broken
...
involved in each turn of the collaboration process to appropriately respond to
...
Robot's approach on acceptance of the delegated task mitigates the Astronaut's
negative emotion (i.e., worriedness) which makes her to respond positively to
...
to help the Robot to select the proper action by responding positively about
...
questions are unnecessary, she becomes frustrated and impatiently answers the
...
this section, we are going to briefly describe all these mechanisms. However, we
