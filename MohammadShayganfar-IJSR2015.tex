...
In this section, the last two walkthrough examples are focusing on the
...
differences in the result of the underlying mechanisms. We also group some of
...
\noindent\textbf{(Perception)} As shown above, although the first Astronaut's
utterance is the same for both examples (C1 \& D1), in the second example the
Perception mechnaism misses the Astronaut's expressed emotion, i.e., worry. The
...
\noindent\textbf{(Theory of Mind: \textit{Reverse Appraisal \& User Modeling})}
Similar to previous sections, in emotion-awareness example, the Robot uses
...
\textit{urgent} and \textit{unexpected}. However, in the emotion-ignorance
example, the Robot finds most of the anticipated appraisals the same, except
that ignoring the Astroanut's negative emotion would cause interpreting the
event as \textit{controllable} and \textit{expected}. As a result, the Robot
updates the Astronaut's user model and forms beliefs that a) the Astronaut has
\textit{low autonomy} in emotion-awareness example versus \textit{high autonomy}
in emotion-ignorance example, and b) Astronaut is a highly communicative
...
same appriasal values in both examples as \textit{relevant},
\textit{unexpected}, \textit{undesirable}, \textit{urgent}, and
\textit{controllable}.\\

\noindent \textbf{(Motivation: \textit{Motive \& Intention Formation})}
The Robot forms new motives in the same way to what we have discussed in Section
\ref{sec:wt-exp1}. The motives in the emotion-awareness example are formed for
acknowledging the Astronaut's negative emotion, and postponing any question
regarding the required information (e.g., unsatisfied postcondition of the
...
forms corresponsing intentions with respect to these motives. After all, the
...
acknowledge the Astronaut's negative emotion, b) to accpet the delegated task,
and c) to postpone asking questions about the delegation process. Hence, the
Robot provides a proper response (C2) without asking any question about the
delegated task.\\
...
In contrast, in the emotion-ignorance example, not only the Robot does not have
any intention to acknowledge the Astronaut's emotion, but also selects an
incorrect coping strategy to seek information about droping its own current
...
restraint coping strategy because of the similar prior processes (as we
discussed above) waits till the appropriate opportunity (i.e., Astronaut's
mitigated emotion) to ask required questions. Therefore, the Robot tries to
mitigate the Astronaut's negative emotion which helps them to successfully
achieve the goal of a task delegation. Notice that in utterance C4, since the
Robot knows that the Astronaut is still worried (based on the Astronaut's
utterance, C3), it only informs the Astronaut about some potential questions in
the future. Consequently, the Robot's collaborative behavior also mitigates the
Astronaut's negative emotion.\\
...
At last, the Astronaut finds the Robot's response appropriate for the delegated
task. Therefore, the Robot's proper response mitigates the Astronaut's negative
