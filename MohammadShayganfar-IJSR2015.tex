...
\ref{sec:AMCT}) forms beliefs about the task being under the Astronaut's focus
of attention, and also the Astronaut's emotion which is expressed both verbally
and nonverbally by her. The beliefs about the task (i.e., installing the panel)
include:
...
\begin{enumerate}
  ...
  Robot uses reverse appraisal to understand the meaning of frustration
  according to the collaborative task status (e.g., precondition shared goal
  status). Robot updates human's user model respectively.
\end{enumerate}
...
reverse appraisal process and collaboration mechanism, and forms beliefs about
the Astronaut as the one with \textbf{low autonomy} and a \textbf{highly
...
for each individual appraisal variable. The output of these processes provide a
...
\begin{enumerate}[a)]
  ...
  \item and user model of the Astronaut. 
\end{enumerate}
...
\begin{enumerate}
  ...
  emotion, and provide alternative solution. Subsequently, the Robot responds to
  the Astronaut with appropriate utterances appeared in \textit{\textbf{A2}}.
\end{enumerate}
